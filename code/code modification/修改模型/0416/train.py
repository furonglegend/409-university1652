...
parser.add_argument('--use_dense', action='store_true', help='use DenseNet121 instead of ResNet101')
...
if opt.views == 2:
    ...
    model = three_view_net(len(class_names), droprate=opt.droprate, stride=opt.stride, pool=opt.pool,
                           share_weight=opt.share, use_dense=opt.use_dense, circle=return_feature)
