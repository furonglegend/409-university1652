...
import torch
import yaml
from model import two_view_net, three_view_net, ft_net_dense  # 新增 ft_net_dense
...
def load_network(name, opt):
    ...
    if opt.views == 2:
        model = two_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share,  use_dense=opt.use_dense  # 新增参数)
    elif opt.views == 3:
        model = three_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, use_dense=opt.use_dense  # 新增参数)
