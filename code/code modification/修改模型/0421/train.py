...
# 添加在现有参数之间（约第58行，在--use_dense参数前）
parser.add_argument('--resnet', action='store_true', help='use ResNet50 backbone')
parser.add_argument('--vgg', action='store_true', help='use VGG16 backbone')
parser.add_argument('--densenet', action='store_true', help='use DenseNet121 backbone')
        ...
        for phase in ['train']:
            ...
            for data, data2, data3, data4 in zip(dataloaders['satellite'], dataloaders['street'], dataloaders['drone'],
                ...
                # 在训练循环中修改三视角处理逻辑
                    ...
                    if return_feature:
                        # 统一解包方式 
                        if isinstance(outputs, tuple):  # 当return_f=True时
                            fused_logits, fused_feature = outputs
                        else:  # 兼容旧版
                            fused_logits = outputs
                        
                        # 标签处理：假设同一batch内三视角标签相同
                        labels_all = labels.repeat(3)  # [batch*3]
                        if opt.extra_Google:
                        ...
                        # 损失计算
                        loss = 3.0 * criterion(fused_logits, labels_all)
                        if opt.arcface:
                            loss += 0.5 * criterion_arcface(fused_feature, labels_all)
                        
                        # 预测结果分割
                        batch = labels.size(0)
                        preds = fused_logits[:batch].argmax(dim=1)
                        preds2 = fused_logits[batch:2*batch].argmax(dim=1)
                        preds3 = fused_logits[2*batch:3*batch].argmax(dim=1)
...
if opt.views == 2:
    model = two_view_net(len(class_names), droprate=opt.droprate, stride=opt.stride, pool=opt.pool,
       ...
       model = three_view_net(
        ...
        VGG16=opt.vgg,          # 新增参数传递
        ResNet50=opt.resnet,    # 新增参数传递
        DenseNet=opt.densenet,  # 新增参数传递
