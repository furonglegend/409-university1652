...
import time
import os
from model import multi_view_net  # 确保导入新模型
...
parser.add_argument('--resnet', action='store_true', help='use ResNet50')
parser.add_argument('--densenet', action='store_true', help='use DenseNet')
...
parser.add_argument('--vgg', action='store_true', help='use VGG16 backbone')
            ...
            if phase == 'val':
                with torch.no_grad():
                    if opt.views == 2:
                        ...
                        outputs = model(inputs, inputs2, inputs3)
                        outputs2 = outputs[1] if isinstance(outputs, tuple) else outputs
            else:
                if opt.views == 2:
                    outputs, outputs2 = model(inputs, inputs2)
                    ...
                    # 统一调用接口
                    if opt.extra_Google:
                        outputs = model(inputs, inputs2, inputs3, inputs4)
                    else:
                        outputs = model(inputs, inputs2, inputs3)
                    ...
                    if isinstance(outputs, tuple):
                        ...
                        loss = criterion(main_output, labels) + criterion(aux_output, labels2)
                    else:
...
model = multi_view_net(
    ...
    resnet=opt.resnet,    # 明确指定backbone类型
    vgg=opt.vgg,
    densenet=opt.densenet
)

# 删除原有的views条件判断，因multi_view_net已支持多视图处理
...
base_lr = opt.lr * 0.1 if opt.vgg else opt.lr  # 添加在模型初始化之后
optimizer_ft = optim.SGD([
    {'params': base_params, 'lr': 0.1 * base_lr},
...
# 修改学习率调度策略
if opt.vgg:
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=40, gamma=0.1)  # VGG需要更快衰减
else:
