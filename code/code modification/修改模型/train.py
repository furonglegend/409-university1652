...
parser.add_argument('--optim', default='sgd', type=str, choices=['sgd', 'adam','adamw'], help='optimizer')
parser.add_argument('--n_epochs', default=100, type=int, help='number of total epochs')
parser.add_argument('--weight_decay', default=5e-4, type=float, help='weight decay')
parser.add_argument('--model_type', default='resnet', type=str, choices=['resnet', 'efficient', 'convnext'], help='backbone type')
...
if opt.optim.lower() == 'adamw':
    # 针对AdamW优化器的调度策略
    exp_lr_scheduler = lr_scheduler.CosineAnnealingLR(optimizer_ft, T_max=opt.n_epochs)
else:
    # 原始调度策略（保持与之前代码兼容）
