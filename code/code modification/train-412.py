                ...
                if phase == 'val':
                    ...
                    if opt.views == 2:
                        outputs, outputs2 = model(inputs, inputs2)
                         ...
                         outputs, outputs2, outputs3 = model(inputs, inputs2, inputs3)  # 始终传递三个输入
...
# 创建参数分组
ignored_ids = list(map(id, classifier_params))
base_params = filter(lambda p: id(p) not in ignored_ids, model.parameters())

optimizer_ft = optim.SGD([
    {'params': base_params, 'lr': 0.1 * opt.lr},
    {'params': classifier_params, 'lr': opt.lr}  # 包含所有分类器参数
