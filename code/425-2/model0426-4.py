...
class MultiBackboneEnsemble(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, pool='avg'):
        ...
        self.backbones = nn.ModuleDict({
            ...
            'convnext'   : nn.Sequential(*list(convnext_base(pretrained=True).children())[:-1]),
        ...
        convnext_dim  = 768
        resnet_dim    = 2048
        vit_large_dim = self.backbones['vit_large'].embed_dim
    ...
    def forward(self, x):
        ...
        f2 = self.backbones['convnext'](x).view(x.size(0), -1)  # [B, 768]
        f3 = self.backbones['resnet50'](x).view(x.size(0), -1)  # [B, 2048]
        v  = self.backbones['vit_large'].forward_features(x)   # e.g. [B, N, 1024]
        cls = v[:,0]                                           # [B, 1024]
        concat = torch.cat([f1, F.normalize(f2,dim=1), F.normalize(f3,dim=1), F.normalize(cls,dim=1)], dim=1)
        return self.proj(concat)                               # [B, total_dim//2]
...
class two_view_net(nn.Module):
    def __init__(self, class_num, droprate, ...):
        super().__init__()
        # 用融合模块替换原来的 ft_net_ShuffleViT
        self.model_1 = MultiBackboneEnsemble(embed_dim=1536)    # embed_dim 仅用于初始化 proj 层
        self.model_2 = self.model_1 if share_weight else MultiBackboneEnsemble(embed_dim=1536)

        # 你可以把 proj_head 删掉，因为上面已经做了 BNNeck
        # 取代 proj_head
        # MAMBA 度量头
        self.metric_fc = MAMBA(
            embedding_size=self.model_1.proj[-3].num_features,  # total_dim//2
            num_classes=class_num,
            scale=30.0, margin=0.5, easy_margin=False
        )
