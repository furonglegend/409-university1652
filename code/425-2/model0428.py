...
class MultiBackboneEnsemble(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, pool='avg'):
        ...
        convnext_dim = convnext.classifier[1].in_features  # 768
        self.backbones['convnext'] = nn.Sequential(*list(convnext.children())[:-1])

        # --- 2) compute original branch dims & choose Dp ---
        shuffle_dim   = self.backbones['shuffle_vit'].in_channels  # e.g. 1536
        resnet_dim    = 2048
        vit_large_dim = self.backbones['vit_large'].embed_dim     # e.g. 1024
        total_dim     = shuffle_dim + convnext_dim + resnet_dim + vit_large_dim
        Dp = total_dim  # or a smaller value like 512

        # --- 3) per-branch linear map to Dp ---
        self.fc_map = nn.ModuleDict({
            ...
            'vit_large'  : nn.Linear(vit_large_dim, Dp),
        })

        # --- 4) fusion & projection ---
        self.fusion = CrossBranchFusion(feat_dim=Dp)
        self.proj   = nn.Sequential(
            nn.Linear(Dp, total_dim // 2),
            nn.BatchNorm1d(total_dim // 2),
    ...
    def forward(self, x):
        # extract each branch
        f1 = self.backbones['shuffle_vit'](x)                    # [B, shuffle_dim]
        f2 = self.backbones['convnext'](x).view(x.size(0), -1)   # [B, convnext_dim]
        f2 = F.normalize(f2, p=2, dim=1)
        f3 = self.backbones['resnet50'](x).view(x.size(0), -1)   # [B, resnet_dim]
        f3 = F.normalize(f3, p=2, dim=1)
        v  = self.backbones['vit_large'].forward_features(x)    # [B, N, vit_large_dim]
        cls= F.normalize(v[:,0], p=2, dim=1)                    # [B, vit_large_dim]

        # map to common Dp
        f1p = self.fc_map['shuffle_vit'](f1)                    # [B, Dp]
        f2p = self.fc_map['convnext'](f2)                       # [B, Dp]
        f3p = self.fc_map['resnet50'](f3)                       # [B, Dp]
        csp = self.fc_map['vit_large'](cls)                     # [B, Dp]

        # cross-branch attention fusion
        fused = self.fusion([f1p, f2p, f3p, csp])                # [B, Dp]

        # final projection
        return self.proj(fused)                                 # [B, total_dim//2]
...
class two_view_net(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, pool='avg', share_weight=False):
        ...
        # 度量头：复用本地 ArcMarginProduct
        embed_size = self.model_1.proj[0].out_features
    ...
    def forward(self, x1, x2, labels1=None, labels2=None, x_a=None, x_p=None, x_n=None):
        # 分类 logits
        z1 = self.model_1(x1) if x1 is not None else None
        z2 = self.model_2(x2) if x2 is not None else None
