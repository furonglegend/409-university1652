...
from timm import create_model  # 引入 timm 库来创建模型
...
class ft_net(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg'):
        ...
        # —— 替换 ConvNeXt 为 Swin Transformer —— 
        backbone = create_model('swin_base_patch4_window7_224', pretrained=True)  # 选择一个合适的 Swin Transformer 模型
        backbone.head = nn.Identity()  # 去掉原来的分类头
        self.in_channels = backbone.num_features  # Swin Transformer 最后输出的通道数
        self.pool = pool
        self.model = backbone
        self.se = SEBlock(channel=self.in_channels)  # 使用 SE 模块增强特征
    ...
    def forward(self, x):
        x = self.model.forward_features(x)  # 使用 Swin Transformer 提取特征
        x = self.se(x)  # 添加 SE 模块

        # 根据设置的池化方式进行全局池化
        if self.pool == 'avg+max':
            x1 = F.adaptive_avg_pool2d(x, (1, 1))
            x2 = F.adaptive_max_pool2d(x, (1, 1))
            x = torch.cat((x1, x2), dim=1)
        elif self.pool == 'avg':
...
if __name__ == '__main__':
    ...
    print(net)
    input = Variable(torch.FloatTensor(8, 3, 256, 256))  # 输入样本
    output1, output2 = net(input, input)  # 获取输出
    print('net output size:')
    print(output1.shape, output2.shape)  # 打印输出尺寸