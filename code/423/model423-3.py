...
class ClassBlock(nn.Module):
    def __init__(self, input_dim, class_num, droprate, relu=False, bnorm=True, num_bottleneck=512, linear=True, return_f=False):
        ...
        add_block = []
        if linear:
            # Ensure the input dimension matches the output of the previous layer (e.g., 1024)
        ...
        add_block += [nn.Dropout(p=min(droprate * 2, 1.0))]

        add_block = nn.Sequential(*add_block)
        add_block.apply(weights_init_kaiming)

        classifier = []
        classifier += [nn.Linear(num_bottleneck, class_num)]  # Correct output dimension for classifier
...
class ft_net(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg'):
        ...
        # 使用 ConvNeXt 模型替代 ResNet50
        model_ft = models.convnext_base(pretrained=True)  # 使用ConvNeXt骨干网络
        ...
        if pool == 'avg+max':
            ...
            model_ft.gem2 = GeM(dim=1024)  # ConvNeXt 输出通道数为 1024

        self.model = model_ft
        # 添加 SE 模块，输入通道数为 ConvNeXt 输出通道数
        self.se = SEBlock(channel=1024)  # ConvNeXt 的默认输出通道数为 1024
        # 添加额外的全连接层
        self.fc = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.ReLU(),
    ...
    def forward(self, x):
        x = self.model.features(x)  # 提取 ConvNeXt 特征
        # 添加 SE 模块
        x = self.se(x)
