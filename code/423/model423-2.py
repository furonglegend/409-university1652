...
class ft_net(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg'):
        ...
        model_ft = models.efficientnet_b0(pretrained=True)  # 使用 EfficientNet B0 作为骨干网络
        ...
        if pool == 'avg+max':
            ...
            model_ft.gem2 = GeM(dim=1280)  # 修改为 EfficientNet 输出维度

        self.model = model_ft
        self.se = SEBlock(channel=1280)  # 修改为 EfficientNet 输出维度
    ...
    def forward(self, x):
        x = self.model.features(x)  # EfficientNet 提取特征
