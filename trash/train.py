...
parser.add_argument('--gamma', type=float, default=32, help='CircleLoss gamma')
...
def train_model(model, model_test, criterion, optimizer, scheduler, num_epochs=25):
    ...
    if opt.circle:
        criterion_circle = CircleLoss(m=0.25, gamma=opt.gamma)  # gamma = 64 may lead to a better result.
    if opt.triplet:
