...
def load_network(name, opt):
   ...
   # if opt.use_dense:
    ...
    if opt.views == 2:
        model = two_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share,  use_dense=opt.use_dense)
    elif opt.views == 3:
        model = three_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, use_dense=opt.use_dense)
